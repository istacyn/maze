...
void renderSpriteProjection(void)
{
    ...
    for (int i = 0; i < NUM_SPRITES; i++)
    {
        ...
        if (angleSpritePlayer < (FOV_ANGLE / 2) + EPSILON)
        {
            ...
            sprites[i].distance = calculateDistance(sprites[i].x, sprites[i].y, player.x, player.y);
    ...
    for (int i = 0; i < numVisibleSprites; i++)
    {
        ...
        // Calculate the perpendicular distance of the sprite to prevent the fish-eye effect
        ...
        spriteBottomY = (spriteBottomY > SCREEN_HEIGHT) ? SCREEN_HEIGHT : spriteBottomY;
        ...
        for (int x = spriteLeftX; x < spriteRightX; x++)
        {
            ...
            for (int y = spriteTopY; y < spriteBottomY; y++)
            {
                if (x > 0 && x < SCREEN_WIDTH && y > 0 && y < SCREEN_HEIGHT)
                {
                    ...
                    distanceFromTop = y + (spriteHeight / 2) - (SCREEN_HEIGHT / 2);
